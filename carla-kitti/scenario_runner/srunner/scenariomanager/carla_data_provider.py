...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def request_new_batch_actors(model, amount, spawn_points, autopilot=False,
                                 random_location=False, rolename='scenario', walker=False):
        """
        ...
        for i in range(amount):
            # Get vehicle or pedestrian by model
            if walker:
                blueprint = CarlaDataProvider.create_blueprint(model, rolename,  actor_category="pedestrian")
            else:
            ...
            if walker:
                # generate walker actor
                if spawn_point:
                    spawn_point = carla.Transform()
                    spawn_point.location = CarlaDataProvider.get_world().get_random_location_from_navigation()
                    walker_actor_command = SpawnActor(blueprint, spawn_point)
                    batch.append(walker_actor_command)
        ...
        if walker:
            # generate walker ai controller
            walker_batch = []
            for i in range(len(actors)):
                # Get vehicle or pedestrian by model
                blueprint_ai = CarlaDataProvider.create_blueprint("controller.ai.walker", rolename,
                                                                  actor_category="pedestrian")
                spawn_point = actors[i].get_transform()
                walker_controller_actor_command = SpawnActor(blueprint_ai, spawn_point, actors[i])
                walker_batch.append(walker_controller_actor_command)

            walker_controller_actors = CarlaDataProvider.handle_actor_batch(walker_batch)

            assert len(actors) == len(walker_controller_actors)

            # start all the ai controllers
            for walker_controller_actor in walker_controller_actors:
                walker_controller_actor.start()
                collision_flag = 1
                while collision_flag:
                    try:
                        walker_controller_actor.go_to_location(
                            CarlaDataProvider.get_world().get_random_location_from_navigation())
                        walker_controller_actor.set_max_speed(1 + random.random()/2)
                        collision_flag = 0
                    except:
                        collision_flag = 1
                        print("collision occurs... try again...")
            sync_mode = CarlaDataProvider.is_sync_mode()
            if sync_mode:
                CarlaDataProvider._world.tick()
            else:
                CarlaDataProvider._world.wait_for_tick()
            actors += walker_controller_actors
